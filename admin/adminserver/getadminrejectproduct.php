...
if(isset($_POST['adminrejectproductbtn'])){
  $productid = $_POST['fldapproveproductid'];
  $stmt = $conn->prepare("DELETE FROM productsapprovals WHERE fldapproveproductid = ?");
  $stmt->bind_param("i",$productid);
  if($stmt->execute()){
    header('location: ../admin/adminproductsapprovals.php?editmessage=Product Was Rejected, Seller has been notified!');
  }
  else{
  ...
  $rejectproduct = $stmt1->get_result();
}
